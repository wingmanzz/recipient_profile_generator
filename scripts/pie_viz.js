...
function getODAById (rcvId, type) {
  ...
  console.log('Getting Top ODA for: ' + rcvId, type);
	...
	var odaJSON = JSON.parse(res.getBody('utf8'));
	for (var i = 0; i < odaJSON.item_count; i++) {
		var oda = {
      id: odaJSON.items[i].source._id, // eslint-disable-line no-underscore-dangle
			type: 'None',
			oda: odaJSON.items[i].total
    };
  ...
  return new Promise(function(resolve) {
    resolve(recipients.map(function(d) {
      function getData(type) {
        ...
        if (other.oda > 0)
          oda.push(other);
          return oda.map(function(d) { // eslint-disable-line no-shadow
            return {
